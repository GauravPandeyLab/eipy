...
def fmax_score(y_test, y_score, beta=1.0, pos_label=1, return_threshold=True):
    fmax_score, _, _, threshold_fmax = fmax_precision_recall_threshold(y_test, 
                                                                       y_score, 
                                                                       beta=beta, 
                                                                       pos_label=pos_label)
    if return_threshold:
...
def fmax_precision_recall_threshold(labels, y_score, beta=1.0, pos_label=1):
    ...
    precision_scores, recall_scores, thresholds = precision_recall_curve(labels, y_score)

    np.seterr(divide="ignore", invalid="ignore")
    f_scores = (1 + beta**2) * (precision_scores * recall_scores) / ((beta**2 * precision_scores) + recall_scores)
...
def scores(y_true, y_pred, metrics):
    ...
    if metrics is None:
        metrics = {'fmax (minority)': fmax_score,
                   'auc': roc_auc_score}
    ...
    for metric_key, metric in metrics.items():
        ...
        if 'y_pred' in inspect.signature(metric).parameters:
            ...
            metric_threshold_dict[metric_key] = try_metric_with_pos_label(y_true, 
                                                                          (np.array(y_pred) >= .5).astype(int), 
                                                                          metric, 
                                                                          pos_label), 0.5
        # if y_score parameter exists in metric function then y should be probability vector
            ...
            metric_results = try_metric_with_pos_label(y_true, y_pred, metric, pos_label)
            if isinstance(metric_results, tuple):  # if metric includes threshold value as tuple
